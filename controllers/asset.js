...
exports.get = async function (req, res, next) {
	...
	var total = await db.sequelize.query(
		`select count(id) as total from assets 
		where ${option} (assets_type like '%${search}%' or assets_value like '%${search}%' or assigned_team like '%${search}%' )`,
...
exports.delete = async function (req, res, next) {
	await db.sequelize.query(`DELETE FROM assets WHERE id = '${req.body.id}' `)
	res.send({ flag: "success" });
}

exports.getByid = async function (req, res, next) {
	await db.sequelize.query(
		`select * from assets where id = ${req.body.id}`, { type: db.Sequelize.QueryTypes.SELECT })
		.then((result) => {
